...
import pl.wavesoftware.eid.impl.StaticSettingsProvider;
import pl.wavesoftware.eid.impl.DefaultEidMessage;
 ...
 * <p>
 * You can tweak behavior of Eid with {@link EidConfigurator}.
 *
...
public class Eid implements Serializable {
     ...
     */
    public Eid(CharSequence id, CharSequence ref) {
        this.id = validate(id).toString();
    ...
    /**
     * Provides a setting provider to Eid library. It can be used to reconfigure
     * settings programmatically.
     *
     * @return a setting provider interface
     */
    public static SettingsProvider getSettingProvider() {
        return StaticSettingsProvider.getSettingsProvider(FRIEND);
    }
    ...
    public String toString() {
        return getSettings().getFormatter().format(this);
    }

    private synchronized String doGetUniqueId() {
        if (uniqueId == null) {
            return getSettings().getIdGenerator().generateUniqId();
        }
        return uniqueId;
    }

    @SuppressWarnings({"ConstantConditions", "squid:S2589"})
    private static CharSequence validate(CharSequence id) {
        if (id == null
            || (getSettings().getValidator() != null
            && !getSettings().getValidator().isValid(id))) {
    ...
    private static Settings getSettings() {
        return getSettingProvider().getSettings();
    }
